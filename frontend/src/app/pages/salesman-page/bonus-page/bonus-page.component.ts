...
export class BonusPageComponent implements OnInit {
  ...
  dataSource: MatTableDataSource<Bonus> = new MatTableDataSource(ELEMENT_DATA);
  ...
  deleteBonusOfSalesman(
    ...
    this.serviceBonus.deleteBonusOfSalesmanFromOrangeHRM(employeeid, year, value).subscribe(
      () => {},
      () => {},
      () => {
  ...
  private async getContractsFromSalesman() {
    this.serviceOpenCRX.getSalesmanContracts(this.salesmanAccount.uid).subscribe(
      (data) => {
        console.log("SO: ", data);
  ...
  private getSpecificDetailsOfSalesOrders(salesOrders, i: number = 0) {
    ...
    this.serviceOpenCRX.getSalesOrderData(salesOrders[i].contractID).subscribe(
      (data) => {
  ...
  private getPositionOfSalesOrder(salesOrdersData, i: number = 0) {
    ...
    this.serviceOpenCRX.getSalesOrderPosition(salesOrdersData[i][0].contractID).subscribe(
      (data) => {
        // @ts-ignore
        ...
        for (let [index, product] of this.products.entries()) {
          // @ts-ignore
          if (!data_[index] || product.productID !== data_[index]?.productID) {
  ...
  private deleteBonusOfSalesmanFromMongoDB(employeeid: string | number, year: string | number) {
    this.serviceBonus.deleteBonusOfSalesmanFromMongoDB(employeeid, year).subscribe(
      () => {},
      () => {},
      () => {
        alert(
