...
export class SalesmanPageComponent implements OnInit {
    ...
    dataSource: MatTableDataSource<Salesman> = new MatTableDataSource(
        ELEMENT_DATA,
    );
    ...
    openDialog(data: Salesman) {
        const dialogRef = this.dialog.open(SocialperformanceDialogComponent, {
            data,
        });
    ...
    private fetchSalesmenData(): void {
        this.service.getAllSalesman().subscribe(
            ...
            () => {
                this.buttons.push(
                    {
                        title: 'Show all salesman',
                        action: 'this.showAllSalesman()',
                    },
                    {
                        title: 'Show salesmen without bonuses',
                        action: 'this.showSalesmenWithoutBonuses()',
                    },
                    {
                        title: 'Show salesmen with bonuses',
                        action: 'this.showSalesmenWithBonuses()',
                    },
                );
            },
        );
