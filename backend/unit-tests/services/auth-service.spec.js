...
const demouser = new User("testuser", "John", "Doe", "jd@test.com", "secret", false, "ceo");
...
describe("auth-service unit-tests", function() {
  ...
  describe("auth state check test", function() {
    it("true if session is marked as authenticated", async function() {
      const session = {authenticated: true};
      const isAuthenticated = await authService.isAuthenticated(session);
      expect(isAuthenticated).to.be.true;
    });

    it("false if session is marked as not authenticated", async function() {
      const session = {authenticated: false};
      const isAuthenticated = await authService.isAuthenticated(session);
      expect(isAuthenticated).to.be.false;
    });

    it("false if session is not marked", async function() {
      const session = {};
      const isAuthenticated = await authService.isAuthenticated(session);
      expect(isAuthenticated).to.be.false;
    });
